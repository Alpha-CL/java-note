...
public @interface CustomAnnotation {
     ...
     * 注解方法 必须有返回值
     *
     *      基本数据类型        // byte, int, short, long, flout, double, char
     *      字符串数据类型       // String
     *      枚举数据类型        // enum
     *      注解数据类型        // @annotation
     *      数组数据类型        // <?>[]
     ...
     * 则使用该注解时候, 则可以直接传递值
     *
     * eg: CustomAnnotation(method = "foo")
     *     CustomAnnotation(value = "foo") => CustomAnnotation("foo")
     */
    // String value();
     ...
     *
     *
     * 1. 仅当 当前注解 有且仅有一个方法时, 则可以使用 value 作为方法名, 则使用该注解时, 就可以直接传递 值
     *
